...
test('search, status and issue filters compose',()=>{assert.equal(filterVessels(vessels,{search:'singapore',status:'항해 중'}).length,1);assert.equal(filterVessels(vessels,{issueOnly:true}).length,4);assert.equal(filterVessels(vessels,{search:'zzzz'}).length,0);});
test('unknown positions are kept out of coordinates, not mapped to zero',()=>{assert.equal(coordinates(null),'위치 미확인');assert.equal(coordinates([0,0]),'0.00°N  0.00°E');});
