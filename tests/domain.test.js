...
test('search, status and issue filters compose',()=>{assert.equal(filterVessels(vessels,{search:'singapore',status:'sea'}).length,1);assert.equal(filterVessels(vessels,{issueOnly:true}).length,4);assert.equal(filterVessels(vessels,{search:'zzzz'}).length,0);});
