...
const snapshot={fetchedAt:source.fetchedAt,source:'Dataloy',mode:'published-snapshot',voyages:source.rows.map((r,i)=>{
 ...
 return {id:`voyage-${i+1}`,dataloyId:String(r.key),name:r.vessel?.vesselName||'Vessel unavailable',voyage:`${r.voyageHeader?.voyageStartYear??'?'} / ${r.voyageHeader?.voyageNo??'?'}`,reference:String(r.voyageHeader?.referenceNo??''),status:'OPR',start:utc(r.voyageStartDateGMT),end:utc(r.voyageEndDateGMT),ports:(r.portCalls||[]).slice().sort((a,b)=>(a.portCallSequence??Infinity)-(b.portCallSequence??Infinity)).map(p=>({name:p.port?.portName||'Port unavailable',sequence:p.portCallSequence??null,purpose:purpose(p),arrival:eventDate(p,'ARR'),departure:eventDate(p,'DEP'),arrivalFixed:fixed(p,'ARR'),departureFixed:fixed(p,'DEP')}))};
})};
